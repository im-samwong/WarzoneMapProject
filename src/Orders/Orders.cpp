...
public:
    ...
    OrderList(const OrderList& other) {
        for (const auto& order : other.orders) {
            orders.push_back(order->clone());  // Use clone() for deep copy of each order
        }
    }
    ...
    OrderList& operator=(const OrderList& other) {
        if (this != &other) {
            orders.clear();
            for (const auto& order : other.orders) {
                orders.push_back(order->clone());  // Use clone() for deep copy
            }
        }
        return *this;
    }
    ...
    // Getters
    std::size_t getSize() const { return orders.size(); }
